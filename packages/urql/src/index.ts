...
const makeExecuteSource = (
  operation: Operation,
  graphClient$: Promise<MeshInstance>,
  ...
  return make<OperationResult>((observer) => {
    ...
    Promise.resolve(graphClient$)
      .then((graphClient) => {
        if (ended) return
        if (operation.kind === 'subscription') {
          return graphClient.subscribe(document, variables, context, rootValue, operationName)
        }
        return graphClient.execute(document, variables, context, rootValue, operationName)
      })
...
export const graphExchange =
  (getBuiltGraphClient: () => Promise<MeshInstance>): Exchange =>
  ({ forward }) => {
    const graphClient$ = getBuiltGraphClient()
    return (ops$) => {
